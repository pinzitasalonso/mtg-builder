...
- Tiers: `User.tier` is written by the RevenueCat webhook (`app/api/revenuecat/webhook`) and `app/api/subscription/sync`, both via `lib/revenuecat.ts` `isProOnRevenueCat` — **tri-state**: a null (indeterminate) result must never change a stored tier; the webhook returns 500 on null so RevenueCat retries. Clients read tier from `/api/auth/me`. Free limits in `lib/limits.ts`.
- Web paywall: RevenueCat's own paywall (the same dashboard paywall iOS shows), rendered by `@revenuecat/purchases-js` and paid through Paddle (merchant of record — it handles VAT), which RevenueCat drives. `lib/pro-paywall.ts` lazy-loads the SDK, identifies as `String(User.id)` like iOS, and re-syncs through `/api/subscription/sync` after a purchase. `components/GetPro.tsx` renders nothing until the paywall is configured. Entry points mirror iOS: home header, deck cap, AI limit (chat, assistant, search), scan limit. `/pro` is the deep link, and the manage page for a Pro account. The public key comes from `REVENUECAT_WEB_KEY` through `/api/subscription/config`. `webPaywallKey` serves Paddle `pdl_` keys and live `rcb_` keys; sandbox `rcb_sb_` and `test_` keys work only in dev, and `sk_` never. Paddle's test and live keys look alike, so in production `isProOnRevenueCat` refuses Pro that a web store's test purchase paid for (App Store sandbox, i.e. TestFlight, still counts). Until a key is served (`proOnSale()`), the limit messages call Pro "coming soon".
