...
- Web paywall: RevenueCat's own paywall (the same dashboard paywall iOS shows), rendered by `@revenuecat/purchases-js` and paid through Web Billing. `lib/pro-paywall.ts` lazy-loads the SDK, identifies as `String(User.id)` like iOS, and re-syncs through `/api/subscription/sync` after a purchase. `components/GetPro.tsx` renders nothing until the paywall is configured. Entry points mirror iOS: home header, deck cap, AI limit (chat, assistant, search), scan limit. `/pro` is the deep link, and the manage page for a Pro account. The public key comes from `REVENUECAT_WEB_KEY` through `/api/subscription/config`. `webPaywallKey` serves only live `rcb_` keys in production; sandbox `rcb_sb_` and `test_` keys work only in dev, and `sk_` never. Until a key is served (`proOnSale()`), the limit messages call Pro "coming soon".
